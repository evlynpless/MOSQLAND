#!/bin/bash
#SBATCH -p day
#SBATCH -J sc01_grass_r_rfsrc_allPoints_linearRegression.sh
#SBATCH -n 1 -c 16 -N 1
#SBATCH -t 24:00:00 
#SBATCH -o /gpfs/scratch60/fas/powell/esp38/stdout/sc01_grass_r_rfsrc_allPoints_linearRegression.sh.%A_%a.out
#SBATCH -e /gpfs/scratch60/fas/powell/esp38/stderr/sc01_grass_r_rfsrc_allPoints_linearRegression.sh.%A_%a.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=evlyn.pless@yale.edu
#SBATCH --array=1
#SBATCH --mem=80G

####### sbatch  /home/fas/powell/esp38/scripts/MOSQLAND/RF/sc01_grass_r_rfsrc_linearRegression.sh
###### 
...
export OUT_TXT=/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR
...
Env.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/FST_list_NAmRF3_PredictAll.csv", sep=",", header=T)
## select only index and CSE
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
...
#Straight_RF_tune = tune(CSE ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Needleleaf + EvBroadleaf + DecBroadleaf + MiscTrees + Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), case.wt=Kernel.Vector.Final,  data=Env.table)

#Straight_RF_tune$optimal[["mtry"]]
#Straight_RF_tune$optimal[["nodesize"]]                  

Straight_RF = lm(CSE ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Needleleaf + EvBroadleaf + DecBroadleaf + MiscTrees + Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, na.action=c("na.omit"), data=Env.table)
...
pred = predict(Straight_RF, value.raster)

write.csv(pred, "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/prediction0_vec.csv")

pred2 = read.csv("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/prediction0_vec.csv")

head(pred2)

predict.rast=raster(vals=as.vector(pred2$x),  nrows= 1500 , ncols=4140 , xmn=-113.5, xmx=-79, ymn=24, ymx=36.5)
...
writeRaster(pred.cond, "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/prediction.tif", options=c("COMPRESS=DEFLATE","ZLEVEL=9") , format="GTiff", overwrite=TRUE  )
...
Env.table <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/FST_list_NAmRF3_Iter",ITER,"LeastPathAll.csv"), sep=",", header=T, fill=TRUE)
head(Env.table)
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
head(Gen.table)
...
#LeastPath_RF_tune = tune(CSE ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Needleleaf + EvBroadleaf + DecBroadleaf + MiscTrees + Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), case.wt=Kernel.Vector.Final, data=Env.table)

#LeastPath_RF_tune$optimal[["mtry"]]
#LeastPath_RF_tune$optimal[["nodesize"]]

LeastPath_RF = lm(CSE ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Needleleaf + EvBroadleaf + DecBroadleaf + MiscTrees + Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, na.action=c("na.omit"), data=Env.table)

summary(LeastPath_RF)
...
pred = predict(LeastPath_RF, value.raster)

write.csv(pred, paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/prediction", ITER, "_vec.csv"))                                                                                      
pred2 = read.csv(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_LR/prediction", ITER, "_vec.csv"))                                                                                     
head(pred2) 
