...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,"_StraightRFScatter.pdf"), 5, 5)
...
save.image(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,".RData"))

d = data.frame(RSQ = RSQ_vec, RSQeq = RSQeq_vec, MSE = MSE_vec, MSEeq = MSEeq_vec) 
write.csv(d, paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,"_ValidationTable.csv"), row.names =FALSE)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,"_BestPred.pdf"), 5, 5)
plot(get(ResistanceMap))
dev.off()

fit = lm(get(RF)$predicted ~ LcpLoopDF$FST_arl)
adjr2 = round(summary(fit)$adj.r.squared, digits=3)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,"_BestModelScatter.pdf"), 5, 5)
plot(LcpLoopDF$FST_arl, get(RF)$predicted,  xlab ="Observed FST", ylab="Predicted FST")
legend("bottomright", legend=c(paste0("Adj. R^2 = ", adjr2)), cex=0.7)
dev.off()

pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/SW2/FullData_Run",run,"_ImpVars.pdf"), 5, 5)
varImpPlot(get(RF))
dev.off()
