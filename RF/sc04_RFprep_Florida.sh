...
access = raster("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/access/accessibility_to_cities_2015_v1.0.tif")
...
points = extract(access, dat, sp=T)

points_buffer = extract(access, dat, buffer=3, fun=mean, sp=T)

head(points)

write.table(points, paste0(OUTDIR,"RF_out_access.txt"))

write.table(points_buffer, paste0(OUTDIR, "RF_out_buffer_access.txt"))
