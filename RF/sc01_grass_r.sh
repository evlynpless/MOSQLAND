...
#Update FST_list_NAmRF3.csv so it has the new Houston data >> now FST_list_NAmRF4_Houston_linux.csv

echo "index,V1,V2,locality1,locality2,lat1,long1,lat2,long2,FST_lin,CSE,Resd" > $OUT_TXT/FST_list_Test$point.csv
awk -v point=$point  -F ","  '{ if ($2==point || $3==point ) print }' $OUT_TXT/FST_list_NAmRF4_Houston_linux.csv  >> $OUT_TXT/FST_list_NAmRF4_Test$point.csv
awk -v point=$point  -F ","  '{ if ($2!=point && $3!=point ) print }' $OUT_TXT/FST_list_NAmRF4_Houston_linux.csv  > $OUT_TXT/FST_list_NAmRF4_Trai$point.csv

#### create the start-end points for each line
#updated column numbers

awk -F "," '{ if(NR>1) printf ("%f %f\n%f %f\n%s\n" , $(NF-4), $(NF-5), $(NF-2), $(NF-3), "NaN NaN" ) }' $OUT_TXT/FST_list_NAmRF4_Test$point.csv > $OUT_TXT/FST_line_NAmRF4_Test$point.txt
awk -F "," '{ if(NR>1) printf ("%f %f\n%f %f\n%s\n" , $(NF-4), $(NF-5), $(NF-2), $(NF-3), "NaN NaN" ) }' $OUT_TXT/FST_list_NAmRF4_Trai$point.csv > $OUT_TXT/FST_line_NAmRF4_Trai$point.txt 
...
v.in.lines input=$OUT_TXT/FST_line_NAmRF4_Test$point.txt output=FST_line_NAmRF4_Test$point  separator=" " --overwrite
v.in.lines input=$OUT_TXT/FST_line_NAmRF4_Trai$point.txt output=FST_line_NAmRF4_Trai$point  separator=" " --overwrite
echo addtable for Test and Trai 
v.db.addtable map=FST_line_NAmRF4_Trai$point   
v.db.addtable map=FST_line_NAmRF4_Test$point   
echo  extract mean for straight line in FST_line_NAmRF4_Test$point and FST_line_NAmRF4_Trai$point
for VAR in Trai Test ; do 
export VAR=$VAR
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Needleleaf,EvBroadleaf,DecBroadleaf,MiscTrees,Shrubs,Herb,Crop,Flood,Urban,Snow,Barren,Water,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP,kernel100" > $OUT_TXT/FST_list_NAmRF4_Predict${VAR}$point.csv 
awk -F "," '{ if (NR>1)  print NR-1 , $1   }'   $OUT_TXT/FST_list_NAmRF4_${VAR}$point.csv | xargs -n 2 -P $CPU  bash -c $' 
CAT=$1
INDEX=$2 
v.to.rast  input=FST_line_NAmRF4_${VAR}$point where="cat == $CAT " output=raster$CAT  use="cat" --o   2>/dev/null 
...
' _   >> $OUT_TXT/FST_list_NAmRF4_Predict${VAR}$point.csv 
done
EOF
...
paste -d ","  <(awk -F , '{  if(NR>1) print $2 }' $OUT_TXT/FST_list_NAmRF4_Test$point.csv | uniq )   <(gdallocationinfo -geoloc -wgs84  -valonly   $IN_MSQ/consland/kernel/KernelRas_100m_fnl.tif  $(awk -F , '{  if(NR>1) print  $(NF-4), $(NF-5) }'   $OUT_TXT/FST_list_NAmRF4_Test$point.csv | uniq )) >  $OUT_TXT/FST_list_NAmRF4_KernelTest$point.csv 

awk -F , '{ if(NR>1) print  $(NF-4), $(NF-5) }'   $OUT_TXT/FST_list_NAmRF4_Trai$point.csv | uniq > $OUT_TXT/FST_list_NAmRF4_LatLongTrai$point.txt 
paste -d ","  <(awk -F , '{ if(NR>1) print $2 }'  $OUT_TXT/FST_list_NAmRF4_Trai$point.csv | uniq) <(gdallocationinfo -geoloc -wgs84  -valonly   $IN_MSQ/consland/kernel/KernelRas_100m_fnl.tif  < $OUT_TXT/FST_list_NAmRF4_LatLongTrai$point.txt )   >  $OUT_TXT/FST_list_NAmRF4_KernelTrai$point.csv
rm $OUT_TXT/FST_list_NAmRF4_LatLongTrai$point.txt  
...
Env.table.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_PredictTrai" , point , ".csv"), sep=",", header=T)
## select only index and CSE
Gen.table.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Trai", point         , ".csv"), sep=",", header=T)[,c( 1, 11)]
Env.table.test <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_PredictTest" , point , ".csv"), sep=",", header=T)
Gen.table.test <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Test", point         , ".csv"), sep=",", header=T)[,c( 1, 11)]
###  Rename columns 
...
writeRaster(pred.cond,"/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/prediction.tif",options=c("COMPRESS=DEFLATE","ZLEVEL=9") , format="GTiff", overwrite=TRUE  )                                  
...
awk -F "," '{ if(NR>1) print $1 , $(NF-5),  $(NF-6) ,  $(NF-3),  $(NF-4) }' $OUT_TXT/FST_list_NAmRF4_Test$point.csv | uniq > $OUT_TXT/FST_line_NAmRF4_StartStopTest$point.txt
awk -F "," '{ if(NR>1) print $1 , $(NF-5),  $(NF-6) ,  $(NF-3),  $(NF-4) }' $OUT_TXT/FST_list_NAmRF4_Trai$point.csv | uniq > $OUT_TXT/FST_line_NAmRF4_StartStopTrai$point.txt
...
for VAR in Trai Test ; do 
export VAR=$VAR
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Needleleaf,EvBroadleaf,DecBroadleaf,MiscTrees,Shrubs,Herb,Crop,Flood,Urban,Snow,Barren,Water,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP,kernel100" > $OUT_TXT/FST_list_NAmRF4_Iter${ITER}LeastPath${VAR}$point.csv
cat $OUT_TXT/FST_line_NAmRF4_StartStop${VAR}$point.txt   | xargs -n 5 -P $CPU  bash -c $'
INDEX=$1 
...
' _  >> $OUT_TXT/FST_list_NAmRF4_Iter${ITER}LeastPath${VAR}$point.csv
done
EOF
...
Env.table.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Iter",ITER,"LeastPathTrai",point,".csv"), sep=",", header=T)
Gen.table.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Trai",point,".csv"), sep=",", header=T)[,c( 1, 11)]
Env.table.test <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Iter",ITER,"LeastPathTest",point,".csv"), sep=",", header=T) 
Gen.table.test <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/FST_list_NAmRF4_Test",point, ".csv"), sep=",", header=T)[,c( 1, 11)]
### Add genetic distance column to the new data frame
###  Rename columns 
names(Env.table.train) <- c("index","arid","access","prec","mean.temp","human.density","friction","min.temp","Needleleaf","EvBroadleaf","DecBroadleaf","MiscTrees",
