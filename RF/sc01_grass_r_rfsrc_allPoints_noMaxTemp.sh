...
export OUT_TXT=/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp
...
Env.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_PredictAll.csv", sep=",", header=T)
## select only index and CSE
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
...
nrow(Kernel)
tail(Kernel)
Pairs <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_all.csv", sep=",", header=T)
tail(Pairs)
...
tail(Kernel2)
Pairs2 <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_all.csv", sep=",", header=T)
head(Pairs2)
...
Straight_RF
pdf("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/ErrVIMP_iter0.pdf", 7, 7)
...
pdf("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/Scatter_iter0.pdf", 5, 5)
...
dev.off()
writeRaster(pred.cond, "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/prediction.tif", options=c("COMPRESS=DEFLATE","ZLEVEL=9") , format="GTiff", overwrite=TRUE  )
EOF
...
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Needleleaf,EvBroadleaf,DecBroadleaf,MiscTrees,Shrubs,Herb,Crop,Flood,Urban,Snow,Barren,Water,Slope,Altitude,PET,DailyTempRange,AnnualTempRange,precwet,precdry,GPP,kernel100" > ${OUT_TXT}/FST_list_NAmRF3_Iter${ITER}LeastPathAll.csv
...
Env.table <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_Iter",ITER,"LeastPathAll.csv"), sep=",", header=T)
head(Env.table)
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
head(Gen.table)
...
LeastPath_RF
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/ErrVIMP_iter", ITER, ".pdf"), 7, 7)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/Scatter_iter", ITER, ".pdf"), 5, 5)
plot(Env.table$CSE, LeastPath_RF$predicted.oob,  xlab ="Observed CSE", ylab="Predicted CSE")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(R,3))), cex=0.7)
dev.off()
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noMaxTemp/Scatter2_iter", ITER, ".pdf"), 5, 5)
plot(Env.table$CSE, LeastPath_RF$predicted.oob, xlab ="Observed CSE", ylab="Predicted CSE")
abline(a=0, b=1)
