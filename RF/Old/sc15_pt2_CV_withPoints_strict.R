...
Test.table <- read.table(file=paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/Test_Fold", foldnum, ".csv"), sep=",", header=T)

Train.table <- read.table(file=paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/Train_Fold", foldnum, ".csv"), sep=",", header=T)

#For train data
#create dataframes of begin and end coordinates from a file:
begin.table <- Train.table[,c(7,6)]
begin.coord <- begin.table
coordinates(begin.coord) <- c("long1", "lat1")

end.table <- Train.table[,c(9,8)]
end.coord <- end.table
...
#For test data
begin.table <- Test.table[,c(7,6)]
begin.coord <- begin.table
coordinates(begin.coord) <- c("long1", "lat1")

end.table <- Test.table[,c(9,8)]
end.coord <- end.table
coordinates(end.coord) <- c("long2", "lat2")
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_StraightRF_TrainingScatter.pdf"), 5, 5)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_StraightRF_ValidScatter.pdf"), 5, 5)
...
#save.image(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points/LinFSTData_beforeLCP_Fold",foldnum,".RData"))


#Prepare points for use in least cost path loops - Training
P.points1.train <- SpatialPoints(Train.table[,c(7,6)])
P.points2.train <- SpatialPoints(Train.table[,c(9,8)])
...
P.points1.test <- SpatialPoints(Test.table[,c(7,6)])
P.points2.test <- SpatialPoints(Test.table[,c(9,8)])
...
write.csv(d, paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run", foldnum, "_ValidationTable.csv"), row.names =FALSE)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_BestCor2_Pred_it",best_it,".pdf"), 5, 5)
plot(get(ResistanceMap))
dev.off()

fit = lm(get(RF)$predicted ~ LcpLoopDF.train$FST_lin)
#adjr2 = round(summary(fit)$adj.r.squared, digits=3)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_BestCor2_TrainingScatter_it", best_it, ".pdf"), 5, 5)
...
#adjr2 = round(summary(fit)$adj.r.squared, digits=3)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_BestCor2_ValidScatter_it", best_it,".pdf"), 5, 5)
plot(LcpLoopDF.test$FST_lin, predict(get(RF), LcpLoopDF.test),  xlab ="Observed FST* (valid)", ylab="Predicted FST")
#legend("bottomright", legend=c(paste0("Adj. R^2 = ", adjr2)), cex=0.7)
dev.off()

pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Run",foldnum,"_BestCor2_ImpVars_it",best_it,".pdf"), 5, 5)
varImpPlot(get(RF))
dev.off()

save.image(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/CV_points_strict/LinFSTData_Fold",foldnum,".RData"))
