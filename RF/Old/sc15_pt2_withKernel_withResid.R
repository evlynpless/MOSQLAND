...
StraightMeanDF.train$Resd <- Train.table$Resd
...
RMSE2 = sqrt(mean((predict(Straight_RF, StraightMeanDF.test) - StraightMeanDF.test$Resd)^2))
MAE = mean(abs(Straight_RF$predicted - StraightMeanDF.train$Resd))
MAE2 =  mean(abs(predict(Straight_RF, StraightMeanDF.train) - StraightMeanDF.train$Resd))
MAE3 = mean(abs(predict(Straight_RF, StraightMeanDF.test) - StraightMeanDF.test$Resd))
#Cor1 = cor(predict(Straight_RF, StraightMeanDF.train), StraightMeanDF.train$Resd)
Cor1 = cor(Straight_RF$predicted, StraightMeanDF.train$Resd)
Cor2 = cor(predict(Straight_RF, StraightMeanDF.test), StraightMeanDF.test$Resd)
...
fit = lm(Straight_RF$predicted ~ StraightMeanDF.train$Resd)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_StraightRF_TrainingScatter.pdf"), 5, 5)
plot(StraightMeanDF.train$Resd, Straight_RF$predicted,  xlab ="Observed GD (training)", ylab="Predicted GD")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(Cor1,3))), cex=0.7)
dev.off()

fit = lm(predict(Straight_RF, StraightMeanDF.test) ~ StraightMeanDF.test$Resd)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_StraightRF_ValidScatter.pdf"), 5, 5)
plot(StraightMeanDF.test$Resd, predict(Straight_RF, StraightMeanDF.test),  xlab ="Observed GD (testing)", ylab="Predicted GD")
...
RMSE2 = sqrt(mean((predict(LCP_RF, LcpLoopDF.test) - LcpLoopDF.test$Resd)^2))
MAE = mean(abs(LCP_RF$predicted - LcpLoopDF.train$Resd))
MAE2 =  mean(abs(predict(LCP_RF, LcpLoopDF.train) - LcpLoopDF.train$Resd))
MAE3 = mean(abs(predict(LCP_RF, LcpLoopDF.test) - LcpLoopDF.test$Resd))
#Cor1 = cor(predict(LCP_RF, LcpLoopDF.train), LcpLoopDF.train$Resd)
Cor1 = cor(LCP_RF$predicted, LcpLoopDF.train$Resd)
Cor2 = cor(predict(LCP_RF, LcpLoopDF.test), LcpLoopDF.test$Resd)
...
write.csv(d, paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold", foldnum, "_ValidationTable.csv"), row.names =FALSE)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_BestCor2_Pred_it",best_it,".pdf"), 5, 5)
plot(get(ResistanceMap))
dev.off()

writeRaster(get(ResistanceMap), paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_BestCor2_Pred_it",best_it,".tif"), options=c("COMPRESS=DEFLATE "),formats=GTiff,overwrite=TRUE)

fit = lm(get(RF)$predicted ~ LcpLoopDF.train$Resd)
#adjr2 = round(summary(fit)$adj.r.squared, digits=3)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_BestCor2_TrainingScatter_it", best_it, ".pdf"), 5, 5)
plot(LcpLoopDF.train$Resd, get(RF)$predicted,  xlab ="Observed GD (train)", ylab="Predicted GD")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(Cor1,3))), cex=0.7)
dev.off()

fit = lm(predict(get(RF), LcpLoopDF.test) ~ LcpLoopDF.test$Resd)
#adjr2 = round(summary(fit)$adj.r.squared, digits=3)
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_BestCor2_ValidScatter_it", best_it,".pdf"), 5, 5)
plot(LcpLoopDF.test$Resd, predict(get(RF), LcpLoopDF.test),  xlab ="Observed GD  (test)", ylab="Predicted GD")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(Cor2,3))), cex=0.7)
dev.off()

pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/ResidData_Fold",foldnum,"_BestCor2_ImpVars_it",best_it,".pdf"), 5, 5)
varImpPlot(get(RF))
dev.off()

save.image(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc15/sc15_pt2_withKernel_withResid",foldnum,".RData"))
