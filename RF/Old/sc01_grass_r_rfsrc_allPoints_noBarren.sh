#!/bin/bash
#SBATCH -p day
#SBATCH -J sc01_grass_r_rfsrc_allPoints_noBarren.sh
#SBATCH -n 1 -c 16 -N 1
#SBATCH -t 24:00:00
#SBATCH -o /gpfs/scratch60/fas/powell/esp38/stdout/sc01_grass_r_rfsrc_allPoints_noBarren.sh.%A_%a.out
#SBATCH -e /gpfs/scratch60/fas/powell/esp38/stderr/sc01_grass_r_rfsrc_allPoints_noBarren.sh.%A_%a.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=evlyn.pless@yale.edu
#SBATCH --array=1
#SBATCH --mem=80G

####### sbatch  /home/fas/powell/esp38/scripts/MOSQLAND/RF/sc01_grass_r_rfsrc_noBarren.sh
######
...
export OUT_TXT=/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren
...
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Needleleaf,EvBroadleaf,DecBroadleaf,MiscTrees,Shrubs,Herb,Crop,Flood,Urban,Snow,Water,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP,kernel100" > ${OUT_TXT}/FST_list_NAmRF3_PredictAll.csv
...
Env.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_PredictAll.csv", sep=",", header=T)
## select only index and CSE
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
...
###  Rename columns
names(Env.table) <- c("index","arid","access","prec","mean.temp","human.density","friction","min.temp","Needleleaf","EvBroadleaf","DecBroadleaf","MiscTrees","Shrubs","Herb","Crop","Flood","Urban","Snow","Water","Slope","Altitude","PET","DailyTempRange","max.temp","AnnualTempRange","prec.wet","prec.dry","GPP","kernel100")
...
#"Shrubs","Herb","Crop","Flood","Urban","Snow","Water","Slope","Altitude","PET","DailyTempRange","max.temp","AnnualTempRange","prec.wet","prec.dry","GPP","kernel100")
...
set.seed(NULL)
load(file = "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc17_rasterstack_noBarren.RData")
#Run random forest
...
nrow(Kernel)
tail(Kernel)
Pairs <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_all.csv", sep=",", header=T)
tail(Pairs)
...
tail(Kernel2)
Pairs2 <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_all.csv", sep=",", header=T)
head(Pairs2)
...
Straight_RF
pdf("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/ErrVIMP_iter0.pdf", 7, 7)
...
pdf("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/Scatter_iter0.pdf", 5, 5)
...
dev.off()
writeRaster(pred.cond, "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/prediction.tif", options=c("COMPRESS=DEFLATE","ZLEVEL=9") , format="GTiff", overwrite=TRUE  )
EOF
...
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Needleleaf,EvBroadleaf,DecBroadleaf,MiscTrees,Shrubs,Herb,Crop,Flood,Urban,Snow,Water,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP,kernel100" > ${OUT_TXT}/FST_list_NAmRF3_Iter${ITER}LeastPathAll.csv
...
Env.table <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_Iter",ITER,"LeastPathAll.csv"), sep=",", header=T)
head(Env.table)
Gen.table <- read.table("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/FST_list_NAmRF3_all.csv", sep=",", header=T)[,c( 1, 11)]
head(Gen.table)
...
LeastPath_RF
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/ErrVIMP_iter", ITER, ".pdf"), 7, 7)
...
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/Scatter_iter", ITER, ".pdf"), 5, 5)
plot(Env.table$CSE, LeastPath_RF$predicted.oob,  xlab ="Observed CSE", ylab="Predicted CSE")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(R,3))), cex=0.7)
dev.off()
pdf(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_noBarren/Scatter2_iter", ITER, ".pdf"), 5, 5)
plot(Env.table$CSE, LeastPath_RF$predicted.oob, xlab ="Observed CSE", ylab="Predicted CSE")
abline(a=0, b=1)
