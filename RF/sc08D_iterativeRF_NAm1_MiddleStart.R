...
LcpExtract <- c()
  ...
  for (x in 1:3) {
      ...
      LcpLoop <-      
  	#foreach(y=(x+1):NumSites, .combine='c', .packages=c('raster', 'gdistance')) %dopar% {

        foreach(y=2:3, .combine='c', .packages=c('raster', 'gdistance')) %dopar% {

     		Ato <- shortestPath(trNAm1C, P.points[x,], P.points[y,], output="SpatialLines")  
     		#assign(paste0("Ato_px",x,"_py",y,"_it",it) , Ato)
		print("shortestPath done")

      		raster::extract(env, Ato, fun=mean, na.rm=TRUE)   
		print("extract done") 
 
    }
}   # vector LcpLoop fill in 
      	     	LcpExtract  <- LcpExtract + LcpLoop
                write.csv(LcpExtract, "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_1/sc08D_LcpLoop1.csv")
