...
#SBATCH -n 1 -c 8 -N 1
#SBATCH -t 1:00:00 
#SBATCH -o /gpfs/scratch60/fas/powell/esp38/stdout/sc01_grass_r2.sh.%J.out 
#SBATCH -e /gpfs/scratch60/fas/powell/esp38/stderr/sc01_grass_r2.sh.%J.err
...
#Copying predict training from Giuseppe for now since mine is still missing rows
...
##Before this, the tables should get reordered according to index "cat" - otherwise CSE won't line up
##Or another way to do with indexing?

Env.table.test = Env.table.test[order(Env.table.test$index),]
Env.table.train = Env.table.train[order(Env.table.train$index),]
...
#first try without tuning
Straight_RF = rfsrc(CSE ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Needleleaf + EvBroadleaf + DecBroadleaf + MiscTrees + Shrubs + Herb + Crop + Flood + Urban + Snow + Barren + Water + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE,  na.action=c("na.omit"), data=Env.table.train)

pred.cond <- 1 / predict.rfsrc(Straight_RF, env) 

#writeRaster is still not working, with or without the COMPRESS
...
#writeRaster(pred.cond,'/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTesting/pred.cond.tif')                          
