#!/bin/bash
#SBATCH -p day
#SBATCH -J sc01_LOOCV.sh
#SBATCH -n 1 -c 16 -N 1
#SBATCH -t 24:00:00
#SBATCH -o /gpfs/scratch60/fas/powell/esp38/stdout/sc01_LOOCV.sh.%A_%a.out
#SBATCH -e /gpfs/scratch60/fas/powell/esp38/stderr/sc01_LOOCV.sh.%A_%a.err
#SBATCH --mail-type=ALL
#SBATCH --mail-user=evlyn.pless@yale.edu
#SBATCH --array=1-38
#SBATCH --mem=80G

####### sbatch  /home/fas/powell/esp38/scripts/MOSQLAND/RF/sc01_LOOCV.sh
...
### Define input and output locations, and set-up parallelizationexport IN_TXT=/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3
...
###  Spliting data into training and testing datasets. Select only one point (all the pairwise from that point) for the testing dataset.
...
awk -v point=$point  -F ","  '{ if ($2==point || $3==point ) print }' $IN_TXT/GeneticDistanceInput.csv  >> ${OUT_TXT}_${point}/FST_list_NAmRF3_Test$point.csv
awk -v point=$point  -F ","  '{ if ($2!=point && $3!=point ) print }' $IN_TXT/GeneticDistanceInput.csv  > ${OUT_TXT}_${point}/FST_list_NAmRF3_Trai$point.csv

#### Create the start-end points for each straight line connecting each pair of sites.
...
### Enter in GRASS and calculate the mean under straight lines
...
load(file = "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc00_prepare_rasterstack.RData")
...
dev.off()
Rtrain = cor(Straight_RF$predicted.oob, Env.table.train$CSE)
...
RMSEtrain = sqrt(mean((Straight_RF$predicted.oob - Env.table.train$CSE)^2))
...
MAEtrain =  mean(abs(Straight_RF$predicted.oob - Env.table.train$CSE))
...
plot(Straight_RF$predicted.oob, Env.table.train$CSE,  xlab ="Predicted CSE (training)", ylab="Observed CSE")
...
plot(predict.rfsrc(Straight_RF, Env.table.test)$predicted, Env.table.test$CSE,  xlab ="Predicted CSE (testing)", ylab="Observed CSE")
...
## Load the testing and trainging data
...
## Load spatial rasterstack
load(file = "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/RF/NAm_RF_3/sc00_prepare_rasterstack.RData")

## Create a vector containing the inverse of the minimum value of the kernel density of the lower density kernel point from each pair
Kernel.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_", point, "/FST_list_NAmRF3_KernelTrai" , point , ".csv"), sep=",", header=F)
names(Kernel.train) <- c('V1', 'kernel')
Pairs.train <- read.table(paste0("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/TrainingTestingRfsrc_", point, "/FST_list_NAmRF3_Trai" , point , ".csv"), sep=",", header=T)
Merge.train <- merge (Kernel.train , Pairs.train  , by = "V1" )
...
## Run random forest: Predictor variables are mean along the least cost path lines through the raster, response variable is genetic dist
...
##Convert to connectivity surface by taking the inverse
pred.cond <- 1/predict.rast.mask

## Save the scatterplots and connectivity surface
...
plot(LeastPath_RF$predicted.oob, Env.table.train$CSE,  xlab ="Predicted CSE (training)", ylab="Observed CSE")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(Rtrain,3))), cex=0.7)
dev.off()
...
plot(predict.rfsrc(LeastPath_RF, Env.table.test)$predicted, Env.table.test$CSE,  xlab ="Predicted CSE (training)", ylab="Observed CSE")
legend("bottomright", legend=c(paste0("Pearson correlation = ", round(Rtest,3))), cex=0.7)
dev.off()
...
#Make sure ocean (noData value) is masked out
...
#Rename the connectivity surface
rm ${OUT_TXT}_${point}/prediction.tif
