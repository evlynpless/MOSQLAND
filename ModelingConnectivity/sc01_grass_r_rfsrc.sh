...
###### Modeling procedure for leave-one-out cross-validation using CSE as genetic distnace. User will need to adjust directories.
...
### Define input location and set-up parallelization
...
###  Spliting data into training and testing. Select only one point (all the pairwise from that point) for the testing dataset.
...
#Load the environmental rasterstack
...
###Optional: tune RF for mtry and nodesize
...
##Record performance and validation metrics
##This format allows easy collection of data from the output file
...
#Make sure ocean (noData values are masked outt)
