...
r.external  input=$RAM/grassdbAll/friction_surface_2015_v1.0_EAfrClip.tif  output=friction    --overwrite
...
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP,kernel100" > ${OUT_TXT}/FST_list_EAfr_PredictAll.csv
...
names(Env.table) <- c("index","arid","access","prec","mean.temp","human.density","friction","min.temp","Slope","Altitude","PET","DailyTempRange","max.temp","AnnualTempRange","prec.wet","prec.dry","GPP", "kernel100")
...
load(file = "/project/fas/powell/esp38/dataproces/MOSQLAND/consland/EAfrica_output/rasterstack_noLC.RData")
...
Straight_RF_tune = tune(FST ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), data=Env.table2)

Straight_RF_tune$optimal[["mtry"]]
Straight_RF_tune$optimal[["nodesize"]]

Straight_RF = rfsrc(FST ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), mtry = Straight_RF_tune$optimal[["mtry"]], nodesize =  Straight_RF_tune$optimal[["nodesize"]], data=Env.table2)  
...
echo "index,arid,access,prec,meantemp,humandensity,friction,mintemp,Slope,Altitude,PET,DailyTempRange,maxtemp,AnnualTempRange,precwet,precdry,GPP" > ${OUT_TXT}/FST_list_EAfr_Iter${ITER}LeastPathAll.csv
...
LeastPath_RF_tune = tune(FST ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), data=Env.table)

LeastPath_RF_tune$optimal[["mtry"]]
LeastPath_RF_tune$optimal[["nodesize"]]

LeastPath_RF = rfsrc(FST ~ arid + access  +   prec  +   mean.temp  +   human.density  +   friction + min.temp + Slope + Altitude + PET + DailyTempRange + max.temp + AnnualTempRange + prec.wet + prec.dry + GPP + kernel100, importance=TRUE, na.action=c("na.omit"), mtry = LeastPath_RF_tune$optimal[["mtry"]], nodesize = LeastPath_RF_tune$optimal[["nodesize"]], data=Env.table)
