...
#kernel100I = raster("/project/fas/powell/esp38/dataproces/MOSQLAND/consland/EAfrica_clips/kernel.tif")
#kernel100 = kernel100I*1
#proj4string(kernel100) <- crs.geo

#Create raster stack named env
env=stack(arid, access, prec, mean.temp, human.density, friction, min.temp, Needleleaf, EvBroadleaf, DecBroadleaf, MiscTrees, Shrubs, Herb, Crop, Flood, Urban, Snow, Barren, Water, Slope, Altitude, PET, DailyTempRange, max.temp, AnnualTempRange, prec.wet, prec.dry, GPP) #cutting kernel for now
